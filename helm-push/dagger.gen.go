...
func invoke(ctx context.Context, parentJSON []byte, parentName string, fnName string, inputArgs map[string][]byte) (any, error) {
	...
	case "HelmPush":
		...
		case "Package":
			...
			return nil, (*HelmPush).Package(&parent, ctx, d, registry)
		default:
	...
	case "":
		return dag.CurrentModule().
			WithObject(
				dag.TypeDef().WithObject("HelmPush").
					WithFunction(
						dag.Function("Package",
							dag.TypeDef().WithKind(Voidkind).WithOptional(true)).
							WithArg("d", dag.TypeDef().WithObject("Directory")).
							WithArg("registry", dag.TypeDef().WithKind(Stringkind)))), nil
	default:
